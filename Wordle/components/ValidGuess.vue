<template>
  ...
  // Inject the Game instance
  const game = inject<Game>("GAME")!;
  
  const modelValue = ref(false); // Use ref instead of defineModel
  const selectedWord = ref<string>("");
  const words = WordList;
  const totalPages = ref<number>(Math.ceil(words.length / 10));
  const updatedWords = ref<string[]>(words);
  const currentPage = ref<number>(1);
  
  const pagedWords = computed(() => {
    const start = (currentPage.value - 1) * 10;
    const end = start + 10;
    return (game.guessedLetters.length === 0 ? words : validWords()).slice(start, end);
  });
